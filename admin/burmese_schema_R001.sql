...
RETURNS TABLE(anchor_id INT, anchor_word VARCHAR(50), pos_in_word INT) AS $$
BEGIN
    RETURN QUERY
    SELECT 
        a.id,
        a.burmese_word,
        (strpos(word_text, a.burmese_word::TEXT) - 1)::INT as pos_val
    FROM burmese_anchor_words a
    WHERE word_text LIKE '%' || a.burmese_word || '%'
    AND length(a.burmese_word) >= 2
    ORDER BY length(a.burmese_word) DESC, pos_val;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    FOR word_rec IN SELECT id, burmese_word FROM burmese_words LOOP
        FOR anchor_rec IN SELECT * FROM find_anchors_in_word(word_rec.burmese_word) LOOP
            ...
            VALUES (word_rec.id, anchor_rec.anchor_id, anchor_rec.pos_in_word)
