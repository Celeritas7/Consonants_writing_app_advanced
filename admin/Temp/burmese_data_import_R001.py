...
Usage:
1. Edit the FILE PATHS section below to point to your Excel files
2. Run: python burmese_data_import.py
3. It will generate CSV files and SQL INSERT statements in the OUTPUT_DIR
...
KG_BOOK_FILE = r"D:\Coding\#App_generation\Language_study\#Consonants\Consonants_writing_app_advanced\admin\KG_book_Burmese.xlsx"
WORDS_FILE = r"D:\Coding\#App_generation\Language_study\#Consonants\Consonants_writing_app_advanced\admin\Burmese_Words_R002.xlsx"
RECIPES_FILE = r"D:\Coding\#App_generation\Language_study\#Consonants\Consonants_writing_app_advanced\admin\Recipies_R002.xlsx"

# Output directory (will be created if it doesn't exist)
OUTPUT_DIR = r"D:\Coding\#App_generation\Language_study\#Consonants\Consonants_writing_app_advanced\admin\supabase_output"
# ============================================================
...
def main():
    ...
    if os.path.exists(KG_BOOK_FILE):
        ...
        print(f"Warning: KG Book file not found: {KG_BOOK_FILE}")
    
    if os.path.exists(WORDS_FILE):
        all_data.append(extract_burmese_words(WORDS_FILE))
    else:
        print(f"Warning: Words file not found: {WORDS_FILE}")
    
    if os.path.exists(RECIPES_FILE):
        all_data.append(extract_recipes(RECIPES_FILE))
    else:
        print(f"Warning: Recipes file not found: {RECIPES_FILE}")
    
    if not all_data:
        print("Error: No data files found!")
        print("\nPlease edit the file paths at the top of this script:")
        print("  KG_BOOK_FILE = r'your_path_here'")
        print("  WORDS_FILE = r'your_path_here'")
        print("  RECIPES_FILE = r'your_path_here'")
        return
    ...
    # Generate outputs
    sql_file = os.path.join(OUTPUT_DIR, "burmese_data_import.sql")
    generate_sql_inserts(all_words, anchors, sql_file)
    generate_csv_files(all_words, anchors, OUTPUT_DIR)
...
Files generated:
  - burmese_data_import.sql  (SQL INSERT statements)
  - burmese_categories.csv   (Categories data)
  - burmese_words.csv        (Words data)
  - burmese_anchor_words.csv (Anchor words data)

Next steps:
1. Run burmese_schema.sql in Supabase SQL Editor (creates tables)
2. Run burmese_data_import.sql in Supabase SQL Editor (imports data)
   OR
   Import the CSV files using Supabase Dashboard → Table Editor → Import
